...
Address Config::AddressMapping(uint64_t hex_addr) const {
    hex_addr >>= shift_bits;
    if (ch_pos == -1) { // Non-power-of-2 channels mapping
        // Hard-coded to rocorababgch
        int channel = hex_addr % channels;
        hex_addr /= channels;
        int bg = hex_addr % bankgroups;
        hex_addr /= bankgroups;
        int ba = hex_addr % banks_per_group;
        hex_addr /= banks_per_group;
        int rank = hex_addr % ranks;
        hex_addr /= ranks;
        int co = hex_addr % (columns/BL);
        hex_addr /= (columns/BL);
        int ro = hex_addr % rows;
...
void Config::SetAddressMapping() {
    ...
    if (address_mapping.size() == 14) { // Non-power of 2 channels
        ch_pos = -1;
        std::cout << "[DRAMSim3] Using hardcoded mapping (non-power-of-2)" 
                  << std::endl;
        return;
    }
