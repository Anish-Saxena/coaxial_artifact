...
DDR_onchip_delay = DDR_onchip_delay[:len_without_means]
DDR_QD=DDR_QD[:len_without_means]
DDR_AST=DDR_AST[:len_without_means]
...
barwidth = 0.5
alval=1
iax1.bar(X_axis, DDR_QD,edgecolor='black',alpha=alval, color='lavender',label='Queuing Delay', width=barwidth, zorder=4, bottom=DDR_QD_bottom)
iax1.bar(X_axis, DDR_AST,edgecolor='black',alpha=alval, color='indigo',label='Access Service Time', width=barwidth, zorder=5)

iax1.bar(X_axis, DDR_onchip_delay,edgecolor='black',alpha=alval, color='#FFA500',label='On-chip Time', width=barwidth, zorder=5, bottom = DDR_AST)
for i in range(len(DDR_mlats)):
    if(DDR_mlats[i] >520):
