...
DDR_onchip_delay =  load_object('AS.OCT.BASE.pkl')
CXL_onchip_delay =  load_object('AS.OCT.CXL.pkl')
...
DDR_QD = load_object('AS.QD.BASE.pkl')
CXL_QD = load_object('AS.QD.CXL.pkl')

DDR_AST = load_object('AS.AST.BASE.pkl')
CXL_AST = load_object('AS.AST.CXL.pkl')
CXL_CID = load_object('AS.CID.CXL.pkl')
...
for i in range(len(DDR_mlats)):
    ...
    #else:
    DDR_QD_bottom.append(DDR_AST[i]+DDR_onchip_delay[i])
#CXL_QD =[]
CXL_QD_bottom=[]
CXL_CID_bottom=[]
for i in range(len(CXL_mlats)):
    ...
    CXL_QD_bottom.append(CXL_AST[i]+CXL_CID[i]+CXL_onchip_delay[i])
    CXL_CID_bottom.append(CXL_AST[i]+ CXL_onchip_delay[i])
    #else:
    #CXL_QD.append(0)
    #CXL_QD_bottom.append(0)
    #CXL_CID_bottom.append(0)
...
barwidth = 0.3
alval=1
iax1.bar(X_axis-(barwidth*0.55), DDR_AST,edgecolor='black',hatch='//',alpha=alval, color='indigo', width=barwidth, zorder=5)
...
iax1.bar(X_axis-(barwidth*0.55), DDR_onchip_delay,edgecolor='black',alpha=alval, hatch='//', color=onchip_color, width=barwidth, zorder=4, bottom=DDR_AST)
...
iax1.bar(X_axis+(barwidth*0.55), CXL_AST,edgecolor='black',alpha=alval, color='indigo', width=barwidth, zorder=5, label='Access Service Time',) #redundant, no label, use same color
iax1.bar(X_axis+(barwidth*0.55), CXL_CID,edgecolor='black',alpha=alval, color='#bf77be',label='CXL Interface Delay', width=barwidth, zorder=4, bottom=CXL_CID_bottom)
iax1.bar(X_axis+(barwidth*0.55), CXL_onchip_delay,edgecolor='black',alpha=alval, color=onchip_color,label='On-chip Time', width=barwidth, zorder=4, bottom=CXL_AST)
