...
bool L2_PA = true;
uint64_t L2_PA_ratio=0; // parallel access X % of l2 misses
//uint64_t on_chip_icn_hops[NUM_CPUS+1][NUM_CPUS+1]={
uint64_t on_chip_icn_hops[13][13]={
    {0, 1, 2, 3, 1, 2, 3, 4, 2, 3, 4, 5, 2},
    {1, 0, 1, 2, 2, 1, 2, 3, 3, 2, 3, 4, 1},
    {2, 1, 0, 1, 3, 2, 1, 2, 4, 3, 2, 3, 2},
    {3, 2, 1, 0, 4, 3, 2, 1, 5, 4, 3, 2, 3},
    {1, 2, 3, 4, 0, 1, 2, 3, 1, 2, 3, 4, 3},
    {2, 1, 2, 3, 1, 0, 1, 2, 2, 1, 2, 3, 2},
    {3, 2, 1, 2, 2, 1, 0, 1, 3, 2, 1, 2, 3},
    {4, 3, 2, 1, 3, 2, 1, 0, 4, 3, 2, 1, 4},
    {2, 3, 4, 5, 1, 2, 3, 4, 0, 1, 2, 3, 4},
    {3, 2, 3, 4, 2, 1, 2, 3, 1, 0, 1, 2, 3},
    {4, 3, 2, 3, 3, 2, 1, 2, 2, 1, 0, 1, 4},
    {5, 4, 3, 2, 4, 3, 2, 1, 3, 2, 1, 0, 5},
    {2, 1, 2, 3, 3, 2, 3, 4, 4, 3, 4, 5, 0}
};
...
void print_roi_stats(uint32_t cpu, CACHE* cache)
{
  ...
  if (TOTAL_ACCESS > 0) {
    ...
    if (cache->NAME == "LLC" && cpu == NUM_CPUS - 1) {
      uint64_t tot_miss_all_cpus = 0;
	  uint64_t total_total_miss_lat=0;
      for (uint32_t i = 0; i < NUM_CPUS; i++) {
		total_total_miss_lat += cache->total_miss_latency[i];
        for (uint32_t j = 0; j < NUM_TYPES; j++) {
          tot_miss_all_cpus += cache->roi_miss[i][j];
        }
      }
      //cout << "LLC_AVG_MISS_LAT_CYCLES " << (1.0*(cache->total_miss_latency))/tot_miss_all_cpus << endl;
      cout << "LLC_AVG_MISS_LAT_CYCLES " << (1.0*(total_total_miss_lat))/tot_miss_all_cpus << endl;
      cout << "LLC_MISS_LAT_HIST (in ns):"<< endl;
	  //let's just print once for now, since everything running same workload
	  cout << "LLC parallel accesses: "<<cache->PA_count<<endl;
	  cout << "LLC parallel access hits: "<<cache->PA_hit<<endl;
	  cout<<"(this stat is per core, but just printing once since everything runs same load)"<<endl;
      for(uint32_t i =0;i<200;i++){
        cout<<i*10<<" : "<<cache->lat_hist[cpu][i]<<endl;        
      }
    }
    if (cache->NAME.find("L2C") != std::string::npos) {
      cout<<"ALL_PAM: "<<cache->PAM_count_per_l2c<<", False_positive: "<<cache->PAM_false_positive<<", Flase_negative: "<<cache->PAM_false_negative<<endl;
    }
  }
}
...
//   std::cout<<"num_reads : "<<DRAM.num_reads<<std::endl;
//   std::cout<<"num_writes: "<<DRAM.num_writes<<std::endl;
//   std::cout<<"all_access: "<<DRAM.num_reads+DRAM.num_writes<<std::endl;
//   //infer cycles from core0
//   uint64_t  num_sim_cycles = ooo_cpu[0]->current_cycle;
//   double sim_time_in_s = num_sim_cycles/2400000000;
//   std::cout<<"core_cycles: "<<num_sim_cycles<<", time in sec (assuming 2.4Ghz): "<<sim_time_in_s<<std::endl;
//   double mbw_system_GBps = ((DRAM.num_reads+DRAM.num_writes) * 64*2.4 )/(num_sim_cycles);
//   std::cout<<"AVG MBW of system(GB/s): "<<mbw_system_GBps<<std::endl;
//   std::cout << std::endl;
// }
...
int main(int argc, char** argv)
{
  ...
  std::cout<<"starting with L2 parallel access: "<<L2_PA<<std::endl;
  ...
  if (argc - optind == NUM_CPUS) {
    ...
    for (int i = 0; i < NUM_CPUS; i++) {
      ...
      traces.push_back(get_tracereader(argv[optind], i, knob_cloudsuite));
    }
  }
  ...
  while (std::any_of(std::begin(simulation_complete), std::end(simulation_complete), std::logical_not<uint8_t>())) {
    ...
    for (std::size_t i = 0; i < ooo_cpu.size(); ++i) {
      ...
      if (show_heartbeat && (ooo_cpu[i]->num_retired >= ooo_cpu[i]->next_print_instruction)) {
        ...
        #if ALLOW_PAM == 3
        cout<<"PAM Allow Rate: "<<L2_PA_ratio<<"%"<<endl;
        #endif
