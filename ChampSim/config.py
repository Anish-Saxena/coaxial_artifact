...
#pmem_fmtstr = 'DRAMSim3_DRAM DRAM(2.5, "/nethome/SCAE/DRAMsim3/configs/DDR4_8Gb_x8_3200.ini", "/nethome/SCAE/coaxial/drasimout/");\n'
# pmem_fmtstr = 'DRAMSim3_DRAM DRAM({attrs[frequency]}, "/nethome/SCAE/coaxial/coaxial_cxl_memsys/dramsim3_ini/DDR5_32GB_2ch_4800.ini", "./");\n'
...
#cxl_fmtstr = 'CXL_MEMORY CXL("CXL", {attrs[frequency]}, 12, &DRAM);\n'
cxl_fmtstr = 'CXL_MEMORY CXL("CXL", {attrs[frequency]}, 12, &DRAM);\n'
...
const_names = {
    ...
    'cxl_memory': {
        'io_freq': 'CXL_IO_FREQ',
        "channels" : 'CXL_CHANNELS',
        "rq_size": 'CXL_RQ_SIZE',
        "wq_size": 'CXL_WQ_SIZE',
        "tCXL" : 'tCXL_DRAM_NANOSECONDS',
        "RD_BW" : 'CXL_RD_BW',
        "WR_BW" : 'CXL_WR_BW',
        "RD_Ch_width_bits" : 'CXL_RD_CH_WIDTH_BITS',
        "WR_Ch_width_bits" : 'CXL_WR_CH_WIDTH_BITS'
    },
    'physical_memory': {
...
default_pmem = { 'name': 'DRAM', 'frequency': 3200, 'cxl_mode': 0, 'channels': 1, 'ranks': 1, 'banks': 8, 'rows': 65536, 'columns': 128, 'lines_per_column': 8, 'channel_width': 8, 'wq_size': 64, 'rq_size': 64, 'tRP': 12.5, 'tRCD': 12.5, 'tCAS': 12.5, 'turn_around_time': 7.5 }
default_vmem = { 'size': 8589934592, 'num_levels': 5, 'minor_fault_penalty': 200 }
default_cxlm= { 'name': 'CXL', 'frequency': 2000, 'lower_level': 'DRAM'}
...
if config_file['CXL_MODE']:
    print("Configuring CXL mode")
    config_file['cxl_memory'] = ChainMap(config_file['cxl_memory'], default_cxlm.copy())
    cxl_mem = config_file.get('cxl_memory', {})
    config_file['LLC']['lower_level'] = 'CXL'
    default_llc['lower_level'] = 'CXL'
...
if config_file['CXL_MODE']:
    config_file['cxl_memory']['io_freq'] = config_file['cxl_memory']['frequency'] # Save value
    freqs = list(itertools.chain(
        [cpu['frequency'] for cpu in cores],
        [cache['frequency'] for cache in caches.values()],
        (config_file['cxl_memory']['frequency'],),
...
if config_file['CXL_MODE']:
    for freq,src in zip(freqs, itertools.chain(cores, caches.values(), 
                        (config_file['cxl_memory'],), (config_file['physical_memory'],))):
        src['frequency'] = freq
else:
...
for fill_level in range(1,len(memory_system)+1):
    for k in active_keys:
        if memory_system[k]['lower_level'] != 'DRAM' and memory_system[k]['lower_level'] != 'CXL':
    ...
    active_keys = [memory_system[k]['lower_level'] for k in active_keys if memory_system[k]['lower_level'] != 'DRAM' and memory_system[k]['lower_level'] != 'CXL']
...
for i in reversed(range(len(memory_system))):
    ul = memory_system[i]
    if ul['lower_level'] != 'DRAM' and ul['lower_level'] != 'CXL':
...
with open(instantiation_file_name, 'wt') as wfp:
    ...
    #if config_file['BASE_MEM_MODEL']:
    #    wfp.write('#include "dram_controller.h"\n')
    #else:
    #    wfp.write('#include "dramsim3_wrapper.hpp"\n')
    if config_file['CXL_MODE']:
        wfp.write('#include "cxl_memory.h"\n')
    ...
    #if config_file['BASE_MEM_MODEL']:
    #    wfp.write(pmem_orig_fmtstr.format(attrs=config_file['physical_memory']))
    #else:
    #    wfp.write(pmem_fmtstr.format(attrs=config_file['physical_memory']))
    if config_file['CXL_MODE']:
        wfp.write(cxl_fmtstr.format(attrs=config_file['cxl_memory']))
    for elem in memory_system:
    ...
    if config_file['CXL_MODE']:
            wfp.write(', '.join('&{name}'.format(**elem) for elem in itertools.chain(cores, memory_system, (config_file['cxl_memory'],), (config_file['physical_memory'],))))
    else:
...
with open(constants_header_name, 'wt') as wfp:
    ...
    # wfp.write('#define ALLOW_PAM 1\n')
    #print (config_file['PAM_MODE'])
    wfp.write('#define ALLOW_PAM '+str(config_file['PAM_MODE'])+'\n')
    pam_th = 50
    if(config_file['PAM_MODE']==3):
        pam_th = config_file['PAM_TH']
    wfp.write('#define PAM_TH '+str(pam_th)+'\n')
    # allow_pam='0'
    # if config_file['ALLOW_PAM']:
    #     allow_pam='1'
    # wfp.write('#define ALLOW_PAM '+allow_pam+'\n')
    if config_file['CXL_MODE']:
        num_ops = len(cores) + len(memory_system) + 2
    else:
        num_ops = len(cores) + len(memory_system) + 1
    wfp.write('#define NUM_OPERABLES ' + str(num_ops) + 'u\n')

    if config_file['CXL_MODE']:
        for k in const_names['cxl_memory']:
            if k in ['tCXL', 'RD_BW', 'WR_BW']:
                wfp.write(define_nonint_fmtstr.format(name=k).format(names=const_names['cxl_memory'], config=config_file['cxl_memory']))
            else:
                wfp.write(define_fmtstr.format(name=k).format(names=const_names['cxl_memory'], config=config_file['cxl_memory']))
...
with open('Makefile', 'wt') as wfp:
    ...
    wfp.write('CFLAGS := ' + config_file.get('CFLAGS', '-Wall -O3') + ' -std=gnu99 -g\n')
    ...
    #wfp.write('LDLIBS := ' + config_file.get('LDLIBS', '-L/storage/home/hcoda1/4/SCAE/p-SCAE-0/hybrid_cxl_memsys/DRAMsim3 -ldramsim3') + '\n')
    wfp.write('LDLIBS := ' + config_file.get('LDLIBS', '-L/nethome/SCAE/DRAMsim3 -ldramsim3') + '\n')
    wfp.write('\n')
